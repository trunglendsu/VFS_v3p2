			     ...
			     PetscInt flg)
{
  ...
  int	i, j, k, ln_v, n1e, n2e, n3e, nintp;
...
/*
PetscErrorCode ibm_interpolation_advanced_sediment(UserCtx *user, IBMNodes *ibm)
{
		...
		for(int ibi=0; ibi<NumberOfBodies; ibi++) {
			...
			while (current) {
				...
				Uc.z = (lucat[kp1][jp1][ip1].z * sk1 + lucat[kp2][jp2][ip2].z * sk2 + lucat[kp3][jp3][ip3].z * sk3);
			        ...
			        //extern PetscReal k_ss;	

				//if(wallfunction==1 || wallfunction==2) {
				//     if(bed_roughness)
                                //     {wall_function_roughness_a(user, k_ss, sc, sb, Ua, Uc, &ucat[k][j][i], &ustar[k][j][i], ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);}
                                 //    else {wall_function_roughness_a (user,0.001, sc, sb, Ua, Uc, &ucat[k][j][i], &ustar[k][j][i], ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);}
			//	}
