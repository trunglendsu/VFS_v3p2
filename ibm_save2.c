		...
		for(ibi=0; ibi<NumberOfBodies; ibi++) {
			...
			while (current) {
				...
				if(averaging) {
					...
					if(wallfunction) wall_function_roughness_a (user,0.0001, sc, sb, Ua, Uc, &ucat[k][j][i], &ustar[k][j][i], ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
