...
extern int immersed, NumberOfBodies, ti, tistart, wallfunction;
extern double find_utau_Cabot(double nu,  double u, double y, double guess, double dpdn);
extern PetscErrorCode MyKSPMonitor1(KSP ksp,int n,PetscReal rnorm,void *dummy);
extern int inletprofile;
...
void Solve_K_Omega(UserCtx *user)
{
	...
	extern PetscErrorCode MySNESMonitor(SNES snes,int n,PetscReal rnorm,void *dummy);
