...
PetscErrorCode  Velocity_Gradient(UserCtx *user, int i, int j, int k, Tensor *Vel_Gradient)
{
DM	        da = user->da, fda = user->fda;
DMDALocalInfo	info = user->info;
int	xs = info.xs, xe = info.xs + info.xm;
...
int IB_Nodes_Gross(UserCtx *user)
{
IBMInfo         *ibminfo;
IBMListNode     *current;
int        Number_Of_IB_Nodes= 0;
...
Cmpnts           ***cent;
 ...
 DMDAVecGetArray(fda, user->lCent, &cent);
  ...
  PetscMalloc(processors*sizeof(int),&rec);
  PetscMalloc(processors*sizeof(int),&dis);
  PetscMalloc(processors*sizeof(int),&stride);
 ...
 //Allocate the rec and dis
  ...
  //Allocate the memory for shear stress of IB object
   PetscMalloc(Number_Of_IB_Nodes*sizeof(int),  &current_element);
   ...
   PetscMalloc(Total_IB_Nodes*sizeof(int),  &element_buffer);
 ...
 if (rank == 0)
   {
     ...
     PetscMalloc(ibm->n_elmt*sizeof(int), &nIB);
