...
/*   insertnode(&test, 11); */
	...
	if(!rotatefsi && !movefsi && rstart_flg && !ibm_search )
	{
		...
		DAVecGetArray(da, user->lNvert_o, &nvert_o);
                ...
                DALocalToGlobal(da, user->lNvert_o, INSERT_VALUES, user->Nvert_o);
                DAGlobalToLocalBegin(da, user->Nvert_o, INSERT_VALUES, user->lNvert_o);
                DAGlobalToLocalEnd(da, user->Nvert_o, INSERT_VALUES, user->lNvert_o);
        }
	else 
	if(rotatefsi && ti>tistart && ibi>=NumberOfRotatingBodies) { // only 1 body can be fixed
		DAVecGetArray(da, user->Nvert, &nvert);
		DAVecGetArray(da, user->lNvert_o_fixed, &nvert_o_fixed);
		for (k=lzs; k<lze; k++)
		for (j=lys; j<lye; j++)
		for (i=lxs; i<lxe; i++) {
		...
		DAVecRestoreArray(da, user->lNvert_o_fixed, &nvert_o_fixed);
		
		DAGlobalToLocalBegin(da, user->Nvert, INSERT_VALUES, user->lNvert);
		DAGlobalToLocalEnd(da, user->Nvert, INSERT_VALUES, user->lNvert);
	}
	else {
		DAVecGetArray(da, user->lNvert_tmp[ibi], &nvert_tmp);
		DAVecGetArray(da, user->Nvert, &nvert);
		DAVecGetArray(da, user->lNvert_o_fixed, &nvert_o_fixed);
		for (k=lzs; k<lze; k++)
		for (j=lys; j<lye; j++)
		...
		DAVecRestoreArray(da, user->lNvert_tmp[ibi], &nvert_tmp);
		DAVecRestoreArray(da, user->lNvert_o_fixed, &nvert_o_fixed);
		DAVecRestoreArray(da, user->Nvert, &nvert);
		
		if(ti<=tistart) {
			DALocalToLocalBegin(user->da, user->lNvert_o_fixed, INSERT_VALUES, user->lNvert_o_fixed);
			DALocalToLocalEnd(user->da, user->lNvert_o_fixed, INSERT_VALUES, user->lNvert_o_fixed);
		}
...
PetscErrorCode ibm_interpolation_advanced(UserCtx *user, PetscInt Add_dUndt)
{
	DA	da = user->da, fda = user->fda;
	Cmpnts	***ucont;
	//int ibi;
	...
	DAVecGetArray(da, user->lNvert, &nvert);
	if(les || rans) DAVecGetArray(da, user->lNu_t, &lnu_t);
	...
	for(int tmp=0; tmp<tmp_max; tmp++) { //tmp_begin
		...
		//if(wallfunction==2) DAVecGetArray(fda, user->lUcat_ib, &lucat_ib);
			...
			*/
			while (current) {
				...
				if(inviscid) freeslip (user, sc, sb, Ua, Uc, &ucat[k][j][i], ibm->nf_x[ni], ibm->nf_y[ni], ibm->nf_z[ni]);
					...
					double nu=1./ren;
					if(levelset) nu = mu_water/rho_water;
		...
		//if(wallfunction==2) DAVecRestoreArray(fda, user->lUcat_ib, &lucat_ib);
	...
	DAVecRestoreArray(da, user->lNvert, &nvert);
	if(les || rans) DAVecRestoreArray(da, user->lNu_t, &lnu_t);
