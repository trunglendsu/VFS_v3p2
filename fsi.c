...
PetscErrorCode Closest_NearBndryPt_ToSurfElmt(UserCtx *user, IBMNodes *ibm, SurfElmtInfo *elmtinfo, FSInfo *fsi, int ibi)
{
  DM da = user->da, fda = user->fda;
  DMDALocalInfo	info = user->info;
...
PetscErrorCode fsi_InterceptionPoint(Cmpnts p, Cmpnts pc[8], 
	       ...
	       PetscReal nfz, IBMInfo *ibminfo, int number, 
	       Cmpnts *intp, int *Need3rdPoint)
{
  int 	triangles[3][12];
  Cmpnts   	p1, p2, p3;
...
PetscErrorCode Find_fsi_3rd_interp_Coeff(int i, int j,
					 int k, int elmt,
					 Cmpnts   p,
					 IBMInfo *ibminfo,
					 UserCtx *user, 
...
PetscErrorCode ibm_Surf_stress(UserCtx *user, IBMNodes *ibm, SurfElmtInfo *elmtinfo, int ibi)
{
  DM		da = user->da, fda = user->fda;
...
/*   PetscPrintf(PETSC_COMM_SELF, "CV fluxes, %le %le %le mass %le\n", flux_y, Fp_y,Ft_y, massflux); */
  ...
  GlobalSum_All(&F_y, &F_ySum, PETSC_COMM_WORLD);
  GlobalSum_All(&F_z, &F_zSum, PETSC_COMM_WORLD);
  GlobalSum_All(&Finert_z, &Finert_zSum, PETSC_COMM_WORLD);
  GlobalSum_All(&Finert_y, &Finert_ySum, PETSC_COMM_WORLD);
  GlobalSum_All(&Fint_z, &Fint_zSum, PETSC_COMM_WORLD);
  GlobalSum_All(&Fint_y, &Fint_ySum, PETSC_COMM_WORLD);
  GlobalSum_All(&massflux, &MassFluxSum, PETSC_COMM_WORLD);
  GlobalSum_All(&Finv_z, &Finv_zSum, PETSC_COMM_WORLD);
  GlobalSum_All(&Finv_y, &Finv_ySum, PETSC_COMM_WORLD);
  GlobalSum_All(&Fvis_z, &Fvis_zSum, PETSC_COMM_WORLD);
  GlobalSum_All(&Fvis_y, &Fvis_ySum, PETSC_COMM_WORLD);
...
PetscErrorCode CV_Boundary(UserCtx *user, FSInfo *fsi, int ibi)
{
  int	i, j, k;  
  IBMInfo       *ibminfo;
  IBMListNode   *current;
...
/* Loop around all ibm nodes */
  ...
  while (current) {
    ...
    if (i>=lxs && i<lxe && j>=lys && j<lye && k>=lzs && k<lze) {	        
	rei= 1./user->ren;
	if(levelset) rei = mu_water/rho_water;//mu[k][j][i]/rho[k][j][i];
...
/*       K +  */
	...
	//if(levelset && level[k][j][i]<0) A_x=0, A_y=0, A_z=0;
