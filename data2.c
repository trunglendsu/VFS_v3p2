...
PetscErrorCode TECIOOut_V(UserCtx *user, int only_V)	// seokkoo
{
	...
	if(only_V)   {
		if(cs) I = TECINI112((char*)"Flow", (char*)"X Y Z UU Cs", filen, (char*)".", &FileType, &Debug, &VIsDouble);
		else if(only_V==2) I = TECINI112((char*)"Flow", (char*)"X Y Z UU", filen, (char*)".", &FileType, &Debug, &VIsDouble);
		else I = TECINI112((char*)"Flow", (char*)"X Y Z UU Nv", filen, (char*)".", &FileType, &Debug, &VIsDouble);
	}
	else if(rans/* && rans_output*/) {
		if(levelset) I = TECINI112((char*)"Flow", (char*)"X Y Z U V W UU P Nv K Omega Nut Level", filen, (char*)".", &FileType, &Debug, &VIsDouble);
		else I = TECINI112((char*)"Flow", (char*)"X Y Z U V W UU P Nv K Omega Nut ", filen, (char*)".", &FileType, &Debug, &VIsDouble);
	}
	else if(temperature) {
		I = TECINI112((char*)"Flow", (char*)"X Y Z U V W UU P Nv Temperature", filen, (char*)".", &FileType, &Debug, &VIsDouble);
		
	}
        else  {
                if(cs) I = TECINI112((char*)"Flow", (char*)"X Y Z U V W UU P Cs", filen, (char*)".", &FileType, &Debug, &VIsDouble);
                else if(levelset) {
                        I = TECINI112((char*)"Flow", (char*)"X Y Z U V W UU P Nv Level", filen, (char*)".", &FileType, &Debug, &VIsDouble);
                }
                else I = TECINI112((char*)"Flow", (char*)"X Y Z U V W UU P Nv", filen, (char*)".", &FileType, &Debug, &VIsDouble);
...
*/
		...
		if(!vc) {
			...
			if(!only_V) I = TECDAT112(&III, &x[0], &DIsDouble);
		...
		// xyang 7-17-2011
                if(temperature) {
                        ...
                        PetscReal ***tmprt;
                        Vec Tmprt;
                        DACreateGlobalVector(user->da, &Tmprt);
                        PetscViewer     viewer;
                        sprintf(filen, "tfield%06d_%1d.dat", ti, user->_this);
                        PetscViewerBinaryOpen(PETSC_COMM_WORLD, filen, FILE_MODE_READ, &viewer);
                        VecLoadIntoVector(viewer, Tmprt);
                        PetscViewerDestroy(viewer);
                        DAVecGetArray(user[bi].da, Tmprt, &tmprt);

                        for (k=zs; k<ze-2; k++)
                        for (j=ys; j<ye-2; j++)
                        for (i=xs; i<xe-2; i++) x[ (k-zs) * (IMax-1)*(JMax-1) + (j-ys) * (IMax-1) + (i-xs)] = tmprt[k+1][j+1][i+1];
                        I = TECDAT112(&III, &x[0], &DIsDouble);

                        DAVecRestoreArray(user[bi].da, Tmprt, &tmprt);
                        VecDestroy(Tmprt);
                }
		//
...
int main(int argc, char **argv)
{
	...
	PetscOptionsGetReal(PETSC_NULL, "-dt", &dt, PETSC_NULL); // xyang
